...
async function test() {
  const { count, error: err1 } = await supabase.from('notes').select('*', { count: 'exact', head: true }).eq('subject', 'site_analytics');
  console.log("Count:", count, err1);
  
  const { data, error } = await supabase.from('notes').insert([
    { title: 'session_hit', subject: 'site_analytics', unit: 'pageview', file_url: 'none', file_path: 'test' }
  ]);
